...
export const addToWishList = asyncHandler(async (req, res) => {
  ...
  const inserted_data = await wishListModel.create({
    userId: _id,
    productId: pid,
    price: price,
  });
...
export const deleteFromWishList = asyncHandler(async (req, res) => {
  const { id } = req?.params;
  const deleteResp = await wishListModel.deleteOne({ productId: id });
  console.log("id ", id);
  if (!deleteResp) {
    return res
      .status(409)
  ...
  if (deleteResp?.deletedCount >= 1) {
    res
      .status(200)
      .json({ success: true, message: "product deleted", deleteResp });
  }
});
